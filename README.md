...
function loadcheck()
    if isfile("RebornXer Hub Blox Fruit   "..game.Players.LocalPlayer.Name..".json") then
    else
    writefile("RebornXer Hub Blox Fruit   "..game.Players.LocalPlayer.Name..".json",game:GetService("HttpService"):JSONEncode(_G.SST))
    return
    end
    end
    pcall(function()
        _G.SST = {Select_Weapon = "Melee",Fast_Attack = true,Bring_Mon = true,Auto_Farm_Level = false
        }
    end)
    function LoadSetting()
        if isfile("RebornXer Hub Blox Fruit   "..game.Players.LocalPlayer.Name..".json") then
            -- โหลดไฟล์
            local fileContent = readfile("RebornXer Hub Blox Fruit   "..game.Players.LocalPlayer.Name..".json")
...
do local GUI = game.CoreGui:FindFirstChild("ReBornXerGui");if GUI then GUI:Destroy();end;if _G.Color == nil then
...
function Update:AddWindow(name,logo,keybind)
	...
	local ReBornXerHubGui = Instance.new("ScreenGui")
	ReBornXerHubGui.Name = "ReBornXerGui"
	ReBornXerHubGui.Parent = game.CoreGui
	ReBornXerHubGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
local ListNof = Instance.new("Frame")
	...
	ListNof.Parent = ReBornXerHubGui
 ...
 NotificationFrame.Parent = ReBornXerHubGui
...
------------------------------------------------ Ui Setting
local ReBornXerUi = Update:AddWindow("RebornXer Hub","10039618734",Enum.KeyCode.RightControl)

local Main = ReBornXerUi:AddTab("Main","6026568198")
local Sea = ReBornXerUi:AddTab("Sea Event","")
local Teleport = ReBornXerUi:AddTab("Island","6035190846")
local Shop = ReBornXerUi:AddTab("Shop","6031265976")
local Misc = ReBornXerUi:AddTab("Misc","6034509993")
local Setting = ReBornXerUi:AddTab("","")

local S = ReBornXerUi:AddTabH("Top","14134158045")

------------------------------------------------  Main L

Main:AddSeperatorLeft("Info")
Main:AddLabelLeft("Wecome To RebornXer Hub Script")
Main:AddLabelLeft("Map : Blox Fruit")
...
Main:AddToggleLeft("Auto Farm Level",_G.SST.Auto_Farm_Level,function(value)
            ...
            if _G.SST.Auto_Farm_Level then
                pcall(function()
                    ...
                    if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false then
                        StartMagnet = false
                        CheckQuest()
                        repeat wait() Tween(CFrameQuest) until (CFrameQuest.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.SST.Auto_Farm_Level
                                    ...
                                    if v.Name == Mon then
                                        if string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, NameMon) then
                                            repeat task.wait()
                                                ...
                                                v.Head.CanCollide = false
                                                StartMagnet = true
                                            until not _G.SST.Auto_Farm_Level or v.Humanoid.Health <= 0 or not v.Parent or game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false
                                        else
                                            StartMagnet = false
...
local TypeWeaponList = {"Melee","Sword","Gun","Blox Fruit"}
Main:AddDropdownRight("Select Weapon",TypeWeaponList,_G.SST.Select_Weapon,function(a)
	Select_Weapon = a
	...
	while wait(1) do
		pcall(function()
			for i,v in pairs(game:GetService("Players").LocalPlayer.Backpack:GetChildren()) do
				if v.ToolTip == _G.SST.Select_Weapon then
					Weapon_Select = v.Name
				end
			end
		end)
	end
...
Main:AddToggleRight("Bring Mon",_G.SST.Bring_Mon,function(value)
	Bring_Mon = value
	_G.SST.Bring_Mon = Bring_Mon
	SS()
end)
...
Main:AddToggleRight("Fast Attack",_G.SST.Fast_Attack,function(a)
	Fast_Attack = a
